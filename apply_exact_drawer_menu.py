...
exact_drawer_html = '''  <!-- RIGHT NAVIGATION DRAWER -->
  <aside class="slide-drawer drawer-right" id="rightNavDrawer" aria-label="Main Navigation Menu">
    ...
    <div class="drawer-content">
      ...
      <div class="drawer-collections-grid" style="margin-bottom:1.8rem;">
        ...
        <a href="collections.html?cat=into-the-dawn" class="drawer-col-pill">
        ...
        <a href="collections.html?cat=out-of-the-chrysalis" class="drawer-col-pill">
      ...
      <div class="drawer-section-title">
        ...
        <span class="txt-ar">جاهز للارتداء</span>
      ...
      <!-- 4. Special VIP Services -->
      <div class="drawer-section-title">
        <i data-feather="star" style="width:14px;height:14px;"></i>
        <span class="txt-ar">الخدمات الخاصة وتجربة VIP</span>
        <span class="txt-en">Special Services & VIP Experience</span>
      </div>
      <ul class="drawer-nav-list" style="margin-bottom:1.8rem;">
        <li class="drawer-nav-item">
          <a href="javascript:void(0)" onclick="window.app.closeDrawers(); window.app.openBookingModal();" class="drawer-nav-link">
            <span>
              <span class="txt-ar">حجز موعد قياس في الأتيليه</span>
              <span class="txt-en">Book Private Atelier Fitting</span>
            </span>
            <i data-feather="calendar" style="width:16px;height:16px; opacity:0.6;"></i>
          </a>
        </li>
        <li class="drawer-nav-item">
          <a href="javascript:void(0)" onclick="window.app.closeDrawers(); window.app.openAiStylistModal();" class="drawer-nav-link">
            <span>
              <span class="txt-ar">مستشارة المظهر بالذكاء الاصطناعي</span>
              <span class="txt-en">AI Couture Stylist</span>
            </span>
            <i data-feather="cpu" style="width:16px;height:16px; color:var(--color-accent-gold);"></i>
          </a>
        </li>
      </ul>
...
for fname in files:
    ...
    content = re.sub(r'<!-- RIGHT NAVIGATION DRAWER -->\s*<aside class="slide-drawer drawer-right" id="rightNavDrawer".*?</aside>', exact_drawer_html, content, flags=re.DOTALL)
    if '<aside class="slide-drawer drawer-right" id="rightNavDrawer"' not in content:
        content = re.sub(r'<aside class="slide-drawer drawer-right" id="rightNavDrawer".*?</aside>', exact_drawer_html, content, flags=re.DOTALL)

    with open(fname, 'w', encoding='utf-8') as f:
        f.write(content)

    print(f"Applied exact user-specified drawer menu to {fname}")
