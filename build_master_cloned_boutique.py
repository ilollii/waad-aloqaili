...
print(f"Building Master Cloned Boutique with {len(products)} products, {len(videos)} videos, {len(collections_map)} collections...")
...
def render_product_card(p, custom_cat_label=""):
    ...
    subcat = p.get('subcategory', 'couture')
    ...
    if subcat == 'bridal':
        ...
        badge_html = '<span class="badge-tag" style="background:#0F9D58; color:#FFF;">YAMAL SS26</span>'
    ...
    return f'''
      <article class="product-card" data-id="{p['id']}" data-cat="{subcat}" data-collections="{cols}">
...
# 1. All 106 Gowns Cards
all_cards_str = '\n'.join([render_product_card(p) for p in products])

# 2. Yamal Collection Gowns (29 gowns)
yamal_prods = [p for p in products if 'yamal' in p.get('collections', []) or 'yamal' in p.get('handle', '')]
if not yamal_prods:
    yamal_prods = products[:8]
yamal_cards_str = '\n'.join([render_product_card(p, "YAMAL SS26") for p in yamal_prods[:8]])

# 3. Veil of Renewal Collection Gowns (22 gowns)
veil_prods = [p for p in products if 'veil-of-renewal' in p.get('collections', []) or 'veil' in p.get('handle', '')]
if not veil_prods:
    veil_prods = products[8:16]
veil_cards_str = '\n'.join([render_product_card(p, "VEIL OF RENEWAL") for p in veil_prods[:8]])

# 4. Celestia Collection Gowns (7 gowns)
celestia_prods = [p for p in products if 'celestia' in p.get('collections', []) or 'celestia' in p.get('handle', '')]
if not celestia_prods:
    celestia_prods = products[16:24]
celestia_cards_str = '\n'.join([render_product_card(p, "CELESTIA COUTURE") for p in celestia_prods])

# 5. Elan Vital Collection Gowns (14 gowns)
elan_prods = [p for p in products if 'elan-vital' in p.get('collections', []) or 'elan' in p.get('handle', '')]
if not elan_prods:
    elan_prods = products[24:32]
elan_cards_str = '\n'.join([render_product_card(p, "ÉLAN VITAL") for p in elan_prods[:8]])
...
<head>
  ...
  <style>
    ...
    .video-selector-bar::-webkit-scrollbar {{
      display: none;
    }}
    .video-pill-btn {{
    ...
    .video-pill-btn:first-child {{
      margin-inline-start: 0.25rem;
    }}
    .video-pill-btn:last-child {{
      margin-inline-end: 0.25rem;
    }}
    .video-pill-btn.active, .video-pill-btn:hover {{
      background: var(--color-accent-gold-gradient);
      color: #120820;
      box-shadow: 0 4px 15px rgba(223, 186, 115, 0.4);
    }}
    ...
    /* Video Player Box inside Collection */
...
<body data-lang="ar">
  ...
  <!-- Fashion Magnetic Cursor -->
  <div class="custom-cursor" id="customCursor"></div>

  <!-- =========================================================================
       ANNOUNCEMENT BAR & BILINGUAL ENGINE
       ========================================================================= -->
  <div class="announcement-bar" id="announcementBar">
    ...
    <div class="announcement-slider" id="announcementSlider">
      ...
      <span class="announcement-item">
        <span class="txt-ar">🎬 شاهد أفلام الحملات والكولكشنات الرسمية لدار وعد العقيلي لعام ٢٠٢٦</span>
        <span class="txt-en">🎬 Watch Official 2026 Haute Couture Campaign Films by Waad Aloqaili</span>
      </span>
    </div>
  ...
  <!-- =========================================================================
       CINEMATIC COUTURE HERO VIDEO PLAYER (Official Shopify CDN Videos)
       ========================================================================= -->
  <section class="hero-cinema-section" id="heroCinema">
    ...
    <div class="hero-cinema-content scroll-reveal">
      ...
      <div style="display:flex; gap:1.2rem; justify-content:center; flex-wrap:wrap;">
        <a href="#catalog" class="btn-primary shimmer-gold-effect" onclick="window.app.filterGownsByCat('all')" style="padding:1.1rem 2.8rem;">
          <span class="txt-ar">استكشاف جميع فساتين البوتيك (١٠٦) &larr;</span>
          <span class="txt-en">Explore All 106 Boutique Gowns &rarr;</span>
        </a>
    ...
    <div class="video-selector-bar">
      <button class="video-pill-btn active" onclick="window.app.switchHeroVideo('{video_yamal}', 'Yamal SS26', 'مجموعة يمال ٢٠٢٦', this)">🌊 Yamal</button>
  ...
  <section class="campaign-collection-block" id="yamal">
    <div class="campaign-header-box scroll-reveal">
      ...
      <a href="#catalog" class="campaign-read-more-link" onclick="window.app.filterGownsByCat('yamal')">
        <span class="txt-en">Read more &rarr;</span>
        <span class="txt-ar">استكشاف فساتين يمال بالكامل (٢٩ فستان) &larr;</span>
      </a>
    </div>
  ...
  <section class="campaign-collection-block" id="veil-of-renewal" style="background:var(--color-bg-alt); border-top:1px solid var(--color-border); border-bottom:1px solid var(--color-border);">
    <div class="campaign-header-box scroll-reveal">
      ...
      <a href="#catalog" class="campaign-read-more-link" onclick="window.app.filterGownsByCat('veil-of-renewal')">
        <span class="txt-en">Read more &rarr;</span>
        <span class="txt-ar">استكشاف تشكيلة حجاب التجدد (٢٢ فستان) &larr;</span>
      </a>
    </div>
  ...
  <!-- =========================================================================
       3. ÉLAN VITAL HERO CAMPAIGN BANNER & GOWNS
       ========================================================================= -->
  <section class="elan-vital-hero" id="elan-vital">
    ...
    <div class="elan-vital-content scroll-reveal">
      ...
      <a href="#catalog" class="btn-primary shimmer-gold-effect" onclick="window.app.filterGownsByCat('elan-vital')" style="padding:1.3rem 3.2rem; font-size:0.92rem; letter-spacing:0.12em;">
        <span class="txt-en">DISCOVER THE COLLECTION (14 GOWNS) &rarr;</span>
        <span class="txt-ar">استكشاف فساتين إيلان فيتال (١٤ فستان) &larr;</span>
      </a>
    </div>
  </section>

  <!-- =========================================================================
       4. CELESTIA COLLECTION SECTION
       ========================================================================= -->
  <section class="campaign-collection-block" id="celestia">
    <div class="campaign-header-box scroll-reveal">
      <span class="campaign-sub-title">ROYAL CELESTIAL COUTURE</span>
      <h2 class="campaign-main-title">
        <span class="txt-en">Celestia</span>
        <span class="txt-ar">مجموعة سيليستيا الملكية</span>
      </h2>
      <p class="campaign-desc-text">
        <span class="txt-en">Inspired by the stars and cosmic brilliance, Celestia captures ethereal beauty in shimmering crystal mesh and bespoke silhouettes.</span>
        <span class="txt-ar">مستوحاة من بريق النجوم وسحر الفضاء الملكي، تجسد مجموعة سيليستيا الجمال الخالد بأقمشة الشبك الكريستالي والقصات الفريدة.</span>
      </p>
      <a href="#catalog" class="campaign-read-more-link" onclick="window.app.filterGownsByCat('celestia')">
        <span class="txt-en">Discover Celestia &rarr;</span>
        <span class="txt-ar">استكشاف فساتين سيليستيا &larr;</span>
      </a>
    </div>

    <!-- Celestia Video Preview -->
    <div class="collection-video-preview-box scroll-reveal">
      <video autoplay muted loop playsinline>
        <source src="{video_celestia}" type="video/mp4">
      </video>
    </div>
  ...
  <!-- =========================================================================
       6. UNDER THE SPOTLIGHT: CANNES FILM FESTIVAL
       ========================================================================= -->
  ...
  <!-- =========================================================================
       8. COMPLETE 106 BOUTIQUE GOWNS MASTER CATALOG (بوابة جميع الفساتين الكاملة)
       ========================================================================= -->
  <main class="catalog-section" id="catalog" style="padding-top:6rem;">
    <div class="catalog-header-bar">
      <div class="catalog-info">
        <h2 class="catalog-title" id="catalogSectionTitle">
          <span class="txt-ar">جميع فساتين البوتيك (١٠٦ فساتين حقيقية)</span>
          <span class="txt-en">Complete Boutique Master Collection (106 Gowns)</span>
        </h2>
        <span class="catalog-count" id="productCountLabel">{len(products)} Masterpieces</span>
      </div>

      <div class="catalog-controls">
        <div class="category-chips-list" id="categoryChips">
          <button class="chip-btn active" onclick="window.app.filterGownsByCat('all')">
            <span class="txt-ar">الكل (١٠٦)</span>
            <span class="txt-en">All (106)</span>
          </button>
          <button class="chip-btn" onclick="window.app.filterGownsByCat('yamal')">
            <span class="txt-ar">يمال (٢٩)</span>
            <span class="txt-en">Yamal (29)</span>
          </button>
          <button class="chip-btn" onclick="window.app.filterGownsByCat('veil-of-renewal')">
            <span class="txt-ar">حجاب التجدد (٢٢)</span>
            <span class="txt-en">Veil (22)</span>
          </button>
          <button class="chip-btn" onclick="window.app.filterGownsByCat('elan-vital')">
            <span class="txt-ar">إيلان فيتال (١٤)</span>
            <span class="txt-en">Élan (14)</span>
          </button>
          <button class="chip-btn" onclick="window.app.filterGownsByCat('celestia')">
            <span class="txt-ar">سيليستيا (٧)</span>
            <span class="txt-en">Celestia (7)</span>
          </button>
          <button class="chip-btn" onclick="window.app.filterGownsByCat('bridal')">
            <span class="txt-ar">الزفاف الملكي</span>
            <span class="txt-en">Royal Bridal</span>
          </button>
          <button class="chip-btn" onclick="window.app.filterGownsByCat('soiree')">
            <span class="txt-ar">السهرة والمناسبات</span>
            <span class="txt-en">Soirée</span>
          </button>
          ...
          <button class="chip-btn" onclick="window.app.filterGownsByCat('couture')">
            ...
            <span class="txt-en">Couture</span>
          </button>
        </div>
      </div>
    </div>

    <!-- 106 Full Boutique Gowns Grid -->
  ...
  <!-- =========================================================================
       RIGHT SLIDE-OUT NAVIGATION DRAWER (All Collections, Gowns & Videos)
       ========================================================================= -->
  ...
  <aside class="slide-drawer drawer-right" id="rightNavDrawer" aria-label="Main Navigation Menu">
    ...
    <div class="drawer-content">
      ...
      <div class="drawer-section-title">
        <span class="txt-ar">جميع المجموعات والفساتين (١٠٦ فساتين)</span>
        <span class="txt-en">Collections & Gowns (106 Gowns)</span>
      </div>
      <ul class="drawer-nav-list">
        <li class="drawer-nav-item">
          <a href="#catalog" class="drawer-nav-link" onclick="window.app.filterGownsByCat('all'); window.app.closeDrawers();">
            <span>
              <span class="txt-ar">✨ جميع فساتين البوتيك (١٠٦ فساتين)</span>
              <span class="txt-en">✨ All 106 Boutique Gowns</span>
            </span>
            <span class="drawer-nav-badge">106</span>
          </a>
        </li>
        <li class="drawer-nav-item">
          <a href="#yamal" class="drawer-nav-link" onclick="window.app.filterGownsByCat('yamal'); window.app.closeDrawers();">
            <span>
        ...
        <li class="drawer-nav-item">
          <a href="#veil-of-renewal" class="drawer-nav-link" onclick="window.app.filterGownsByCat('veil-of-renewal'); window.app.closeDrawers();">
            <span>
              <span class="txt-ar">🌸 حجاب التجدد (Veil of Renewal)</span>
              <span class="txt-en">🌸 Veil of Renewal</span>
            </span>
        ...
        <li class="drawer-nav-item">
          <a href="#elan-vital" class="drawer-nav-link" onclick="window.app.filterGownsByCat('elan-vital'); window.app.closeDrawers();">
            <span>
              <span class="txt-ar">💫 إيلان فيتال (Élan vital)</span>
              <span class="txt-en">💫 Élan vital</span>
            </span>
            <span class="drawer-nav-badge">14</span>
          </a>
        </li>
        <li class="drawer-nav-item">
          <a href="#celestia" class="drawer-nav-link" onclick="window.app.filterGownsByCat('celestia'); window.app.closeDrawers();">
            <span>
      ...
      <!-- 2. Campaigns & Video Cinema -->
      <div class="drawer-section-title">
        <span class="txt-ar">أفلام الحملات والتجارب الخاصة</span>
        <span class="txt-en">Campaign Films & Specials</span>
      </div>
      <ul class="drawer-nav-list">
        <li class="drawer-nav-item">
          <a href="#heroCinema" class="drawer-nav-link" onclick="window.app.closeDrawers()">
            <span>
              <span class="txt-ar">🎬 سينما أفلام الكوتور الرسمية (٧ أفلام)</span>
              <span class="txt-en">🎬 Official Campaign Films (7 Films)</span>
            </span>
  ...
  <div class="search-modal" id="searchModal">
    ...
    <div class="search-popular-tags">
      <span>Popular: </span>
      <a href="#catalog" onclick="window.app.filterGownsByCat('yamal'); document.getElementById('searchModal').classList.remove('active'); document.getElementById('drawerBackdrop').classList.remove('active');" style="margin:0 0.5rem; text-decoration:underline; color:var(--color-brand-purple); font-weight:700;">Yamal SS26</a> |
      <a href="#catalog" onclick="window.app.filterGownsByCat('veil-of-renewal'); document.getElementById('searchModal').classList.remove('active'); document.getElementById('drawerBackdrop').classList.remove('active');" style="margin:0 0.5rem; text-decoration:underline; color:var(--color-brand-purple); font-weight:700;">Veil of Renewal</a> |
...
print("Master Cloned Boutique build complete!")
